...
# ==========================================================
# SKRIP OTOMATIS KONFIGURASI DHCP SERVER (NODE: ALDARION)
# (Versi Final Gabungan Soal 2, 5, dan 6)
# ==========================================================

echo "=== [Aldarion] Memulai Konfigurasi DHCP Server Final ==="

# 1. Menginstal paket (jika belum ada)
echo "--> Memastikan isc-dhcp-server terinstal..."
apt-get update -y
apt-get install -y isc-dhcp-server
...
# 3. Membuat file konfigurasi utama /etc/dhcp/dhcpd.conf
# (Ini menimpa file lama dengan konfigurasi lengkap & benar)
echo "--> Menulis file konfigurasi utama /etc/dhcp/dhcpd.conf..."
cat << EOF > /etc/dhcp/dhcpd.conf
# File ini dibuat otomatis oleh skrip

# === OPSI GLOBAL ===
# (Menggunakan DNS Minastir dan Aturan Waktu Soal 6)
option domain-name-servers 192.239.5.2;
option domain-name "k56.com";
max-lease-time 3600; # Batas 1 jam (Soal 6)
authoritative;
...
subnet 192.239.1.0 netmask 255.255.255.0 {
    ...
    range 192.239.1.68 192.239.1.94;
    # Aturan Waktu Baru (Setengah Jam)
    default-lease-time 1800; # (Soal 6)
}
...
subnet 192.239.3.0 netmask 255.255.255.0 {
    ...
    # (MAC Address Khamul dari log 'ip a' Anda)
    host Khamul {
        hardware ethernet 02:42:f2:39:a2:00;
        fixed-address 192.239.3.95;
    }
...
# 5. Melakukan 'Restart Paksa' pada isc-dhcp-server
echo "--> Melakukan 'Restart Paksa'..."
# (Ini untuk membunuh proses 'stuck' yang lama)
PID=$(ps aux | grep 'dhcpd' | grep -v 'grep' | awk '{print $2}')
kill $PID || true
rm /var/run/dhcpd.pid || true
service isc-dhcp-server start
...
# (Menggunakan MariaDB sebagai pengganti MySQL)
# ==========================================================

# --- Variabel Database (Sesuaikan!) ---
DB_NAME="benteng_db"
DB_USER="ksatria"
...
# 1. Instalasi MariaDB (pengganti mysql-server)
apt-get update
apt-get install -y mariadb-server

# 2. Konfigurasi MariaDB agar bisa diakses dari luar (Network)
# Kita menggunakan 'sed' untuk mengedit file secara otomatis
echo "--> Mengkonfigurasi MariaDB agar bisa diakses dari luar (0.0.0.0)..."
# (Lokasi file konfigurasi MariaDB)
sed -i "s/bind-address\s*=\s*127.0.0.1/bind-address = 0.0.0.0/" /etc/mysql/mariadb.conf.d/50-server.cnf

# 3. Restart layanan database
# (Nama layanannya tetap 'mysql' untuk kompatibilitas)
echo "--> Merestart layanan MariaDB..."
service mysql restart
#!/bin/bash
# ==========================================================
# SKRIP OTOMATIS KONFIGURASI DATABASE (NODE: PALANTIR)
# (PERBAIKAN: Menggunakan 'service mariadb restart')
# ==========================================================
...
# 3. Restart layanan database
# (PERBAIKAN: Menggunakan nama layanan 'mariadb')
echo "--> Merestart layanan MariaDB..."
...
# 1. Konfigurasi file .env menggunakan 'sed'
cd ${PROJECT_DIR}
...
# 2. Menulis ulang file konfigurasi Nginx
cat << EOF > /etc/nginx/sites-available/${NODE_NAME}
# BLOK 1: Tangkap akses via IP dan TOLAK
server {
    listen ${LISTEN_PORT};
    server_name ${NODE_IP};
    # Kirim 404 (Not Found) jika diakses via IP
    return 404;
}
